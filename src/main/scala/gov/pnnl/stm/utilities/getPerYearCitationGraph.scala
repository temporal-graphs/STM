...
object getPerYearCitationGraph {
  ...
  def main(args: Array[String]): Unit = {
    ...
    yearArr.foreach(year =>{
     val yearlyPaper :Array[String] =  Source.fromFile(perYearPaperDir + "/" + year + ".txt")
        .getLines().map(paper => paper).toArray
      ...
      val thisYearRefer = sc.textFile(paperRefSummaryFile).filter(line=> localPaperArr.contains(line.split
      (separator)(0)))
    })
